...
config:
	...
	#@curl -i -X POST http://localhost:8001/services/oauth/plugins -F "name=${NAME}" -F "config.client_secret=$$(grep client_secret cred | awk '{print $$NF}')" -F "config.client_id=$$(grep client_id cred | awk '{print $$NF}')" -F "config.secure_cookies=false" -F "config.http_only_cookies=false"
	@curl -i -X POST http://localhost:8001/services/oauth/plugins -F "name=${NAME}" -F "config.secure_cookies=false" -F "config.http_only_cookies=false"
