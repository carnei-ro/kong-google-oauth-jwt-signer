# Kong Google OAuth JWT Signer

# Use

Use with https://github.com/carnei-ro/kong-oauth-jwt-plugin  
Based on: https://github.com/cloudflare/nginx-google-oauth  
Plugin priority: 1000.  

**FROM VERSION 0.0-3 THIS PLUGINS DEPENDS ON THE CAPABILITY OF [INJECT DIRECTIVES INTO MAIN](https://docs.konghq.com/2.0.x/configuration/#injecting-nginx-directives) - IT HAS STARTED IN KONG 2.0**
**FROM VERSION 0.0-7 THIS PLUGINS DEPENDS ON KONG >= 2.1 - lua-resty-openssl 0.6.2**


summary: generate JWT when a google oauth flow is valid  

## Generate client_id and client_secret

- Visit https://console.developers.google.com

## Default values

```yaml
---
plugins:
- name: kong-google-oauth-jwt-signer
  config:
    client_id: <empty> # client_id or env var KONG_GOOGLE_OAUTH_JWT_SIGNER_CLIENT_ID
    client_secret: <empty> # client_secret or env var KONG_GOOGLE_OAUTH_JWT_SIGNER_CLIENT_SECRET
    private_keys: <empty> # private_keys or env var KONG_GOOGLE_OAUTH_JWT_SIGNER_PRIVATE_KEYS
    # private_keys:
    #   12345678-1234-1234-1234-123456789ABC: 
    jwt_validity: 86400
    cookie_name: oauth_jwt
    secure_cookies: true
    http_only_cookies: true
    issuer: Kong
    callback_uri: /_oauth
    callback_scheme: null
    private_key_id: 12345678-1234-1234-1234-123456789ABC
    ssl_verify: true
    unescape_uri: false
    strip_port_from_host: false
```

## Real example

Configure the signer route:  
```yaml
- hosts:
  - example.carnei.ro
  methods: []
  name: auth-google-callback
  paths:
  - /_oauth
  plugins:
  - config:
      client_id: 123your-client-id-here.apps.googleusercontent.com
      client_secret: your-client-secret-here
      private_key_id: key-id-from-jwt_signer_private_keys
    name: kong-google-oauth-jwt-signer
  preserve_host: false
  regex_priority: 1
  service: does-not-matter
  strip_path: false
```

## Requirements

Edit `kong.conf` to configure lua to be able to validate SSL certificates:  
```conf
lua_ssl_trusted_certificate=/etc/pki/ca-trust/extracted/openssl/ca-bundle.trust.crt
lua_ssl_verify_depth=3
```

Edit `kong.conf` to permit expose environment variables `KONG_GOOGLE_OAUTH_JWT_SIGNER_CLIENT_ID`, `KONG_GOOGLE_OAUTH_JWT_SIGNER_CLIENT_SECRET` and/or `KONG_GOOGLE_OAUTH_JWT_SIGNER_PRIVATE_KEYS` to kong:
```conf
nginx_main_include=main.conf
```

`main.conf`
```conf
env KONG_GOOGLE_OAUTH_JWT_SIGNER_CLIENT_ID;
env KONG_GOOGLE_OAUTH_JWT_SIGNER_CLIENT_SECRET;
env KONG_GOOGLE_OAUTH_JWT_SIGNER_PRIVATE_KEYS;
```

Generate a key pair to use in the JWT. (The `public` goes to the `kong-oauth-jwt-plugin` plugin.)  
```bash
openssl genrsa -out private.pem 2048 # generates the private
openssl rsa -in private.pem -outform PEM -pubout -out public.pem  # generates the public
```

Create the environment variable `KONG_GOOGLE_OAUTH_JWT_SIGNER_PRIVATE_KEYS` that is a JSON.
Format:  
- key = `kid` of the JWT 
- value = private key for the JWT in base64.  

To generates the base64 do:
```bash
cat private.pem | base64 | paste -s -d ""
```

Example:
`KONG_GOOGLE_OAUTH_JWT_SIGNER_PRIVATE_KEYS`
```json
{
  "12345678-1234-1234-1234-123456789ABC":""
}
```
